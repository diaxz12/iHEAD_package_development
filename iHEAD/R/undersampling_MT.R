...
list_data <- processamento_dados("MT_Aereas", 1, 2, 2017)
...
#No undersampling
selecao_variaveis(list_data,"MT_Aereas",1, 10, 1, 1, 2, 2017,'square')
results_square <- prever_numero_avarias_MT(list_data,atualizar_modelo="sim",afinamento="nao","Aereas",'square')
write.csv(results_square, file = paste(path,"/No_undersampling_transformations/MT_results_square.csv", sep=""))
selecao_variaveis(list_data,"MT_Aereas",1, 10, 1, 1, 2, 2017,'reciprocal')
results_reciprocal <- prever_numero_avarias_MT(list_data,atualizar_modelo="sim",afinamento="nao","Aereas",'reciprocal')
write.csv(results_reciprocal, file = paste(path,"/No_undersampling_transformations/MT_results_reciprocal.csv", sep=""))


#All recrods with failures equal to 0 : Experience 1
x <- training$Codigo.Circuito[which(training$Falhas_Totais_porKm==0)]
...
training <- training[!(training$Codigo.Circuito %in% no_failures),]
#Check new failure distribution
new_hist <- hist(training$Falhas_Totais_porKm)
list_data$treino <- training

selecao_variaveis(list_data,"MT_Aereas",1, 10, 1, 1, 2, 2017,'log')
results_log <- prever_numero_avarias_AT(list_data,atualizar_modelo="nao",afinamento="nao","Aereas",'log')
write.csv(results_log, file = paste(path,"/Undersampling_transformations_exp1/MT_results_log.csv", sep=""))
selecao_variaveis(list_data,"MT_Aereas",1, 10, 1, 1, 2, 2017,'square')
results_square <- prever_numero_avarias_AT(list_data,atualizar_modelo="nao",afinamento="nao","Aereas",'square')
write.csv(results_square, file = paste(path,"/Undersampling_transformations_exp1/MT_results_square.csv", sep=""))
selecao_variaveis(list_data,"MT_Aereas",1, 10, 1, 1, 2, 2017,'reciprocal')
results_reciprocal <- prever_numero_avarias_AT(list_data,atualizar_modelo="nao",afinamento="nao","Aereas",'reciprocal')
write.csv(results_reciprocal, file = paste(path,"/Undersampling_transformations_exp1/MT_results_reciprocal.csv", sep=""))
...
results_log <- prever_numero_avarias_AT(list_data,atualizar_modelo="nao",afinamento="nao",'log')
...
results_square <- prever_numero_avarias_AT(list_data,atualizar_modelo="nao",afinamento="nao",'square')
...
results_reciprocal <- prever_numero_avarias_AT(list_data,atualizar_modelo="nao",afinamento="nao",'reciprocal')
