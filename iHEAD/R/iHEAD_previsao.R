...
iHEAD_master_algorithm <- function(flag_tipoLinha, flag_prazo, niter, flag_versao=1, horizonte, anosTeste, ano_inicio_estimativa, atualizar_modelo="nao", afinamento="nao"){
  ...
  selecao_variaveis(lista_dados,flag_tipoLinha,flag_prazo, niter, flag_versao, horizonte, anosTeste, ano_inicio_estimativa)

  if(flag_prazo==1){
    if(flag_tipoLinha=="AT"){
      resultados<-prever_numero_avarias_AT(lista_dados,atualizar_modelo,afinamento)
      load(paste(path,"/Resultados/AT_CurtoPrazo/variaveis_melhor_modelo_xgboost_AT.rda",sep = ""))
    }else{
      resultados<-prever_numero_avarias_MT(lista_dados,atualizar_modelo,afinamento,str_sub(flag_tipoLinha,4))
...
#'dados<-pre_processamento_dados("MT_Subterraneas",1,2,2017)
#'@export
processamento_dados<-function(flag_tipoLinha, horizonte, anosTeste, ano_inicio_estimativa){
  ...
  categorical_feats <- names(feature_classes[feature_classes != "numeric"])
  ...
  if("AT" != flag_tipoLinha){
    numeric_feats<-c(numeric_feats, "Falhas_Totais_por_km_y_mais_1", "Comprimento_total_y_mais_1", "Anos_Historico")
    ...
    if("Falhas_Totais_por_km_y_mais_1" %in% colnames(treino))
    {
    ...
    numeric_feats<-numeric_feats[!(numeric_feats %in% c("Falhas_Totais_por_km_y_mais_1", "Comprimento_total_y_mais_1", "Anos_Historico"))]
    if("Falhas_Totais_por_km_y_mais_1" %in% colnames(treino))
    {
      cat("Yep, it's in there!\n");
    }
  ...
  if("AT" != flag_tipoLinha){
    return(list("treino"=treino, "teste"=teste,"areas"=areas,
                ...
                "Distrito_treino"=Distrito_treino,"Distrito_teste"=Distrito_teste,
                "Concelho_treino"=Concelho_treino,"Concelho_teste"=Concelho_teste))
  }else{
...
#'@export
selecao_variaveis<-function(lista_dados,flag_tipoLinha,flag_prazo, niter, flag_versao, horizonte, anosTeste, ano_inicio_estimativa){
  ...
  if("AT" != flag_tipoLinha){
    ...
    treino<-treino[, -grep(colnames(treino),"Codigo.Circuito")]
    teste<-teste[, -grep(colnames(teste),"Codigo.Circuito")]

    variaveis_remover<-c("Codigo.Circuito")
    var_numericas<-var_numericas[!(var_numericas %in% variaveis_remover)]
  }else{
    treino<-treino[, -grep(colnames(treino),"Codigo.da.linha")]
    teste<-teste[, -grep(colnames(teste),"Codigo.da.linha")]
  ...
  if("AT" != flag_tipoLinha){
    ...
    nctr<-unique(Concelho_treino)
    nct<-unique(Concelho_teste)
    ndtr<-unique(Distrito_treino)
    ndt<-unique(Distrito_teste)
  ...
  for(z in 1:ntestes){
    ...
    newv<-c() #vetor que vai conter as variaveis seleccionadas
...
prever_numero_avarias_AT<-function(lista_dados,atualizar_modelo="nao",afinamento="nao"){
...
prever_numero_avarias_MT<-function(lista_dados,atualizar_modelo="nao",afinamento="nao",Tipo_linha_MT){
